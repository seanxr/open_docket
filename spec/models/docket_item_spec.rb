...
describe DocketItem do
  ...
  before do
    @docketitem = DocketItem.new(number: "2012-0001", request: "This is a request", opened_on: "01/01/2012", requested_by: "requestors", ward: "1", precinct: "1", address:"1 Centre Street", creator_id: 1, updater_id: 1) 
  ...
  describe "when number is not present" do
    before { @docketitem.number = " " }
    it { should_not be_valid }
  end

  describe "when number is too long" do
    before { @docketitem.number = "a" * 21 }
    it { should_not be_valid }
  end

  describe "when number is already taken" do
    before do
      docket_item_with_same_name = @docketitem.dup
